...
describe('GET /api/images', () => {
  ...
  it('responds with 200 if called correctly and image exist', (done): void => {
    request(app).get('/api/images?filename=hero&height=100&width=100').expect(200, done)
  })

  it('created a thumb version of the image', (done): void => {
    request(app)
      .get('/api/images?filename=hero&height=100&width=100')
      .then(() => {
        fs.stat(path.resolve(__dirname, '../../../assets/thumb/hero-100x100.jpg')).then((fileStat: Stats) =>
  ...
  it('created a thumb version of the image with the correct height and width', (done): void => {
    request(app)
      .get('/api/images?filename=hero&height=100&width=150')
      .then(() => {
        const dimensions = sizeOf(path.resolve(__dirname, '../../../assets/thumb/hero-100x150.jpg'))
